...
public class MemberDaoImpl extends GenericDaoImpl<Member> implements MemberDao {
	...
	public Member findOneJoinFetch(long id) {
		  Query query =  entityManager.createQuery("SELECT  m FROM Member AS m JOIN FETCH m.addresses AS a where m.id= :member");
		  return (Member) query.setParameter("member", id).getSingleResult();
